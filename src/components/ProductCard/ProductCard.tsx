...
export const ProductCard = ({ product }: ProductCardProps) => {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    const firstAvailableOption = product.options.find((opt) => opt.quantity > 0);
    if (!firstAvailableOption) return;

    const selectedVariant: Record<string, string> = {};
    Object.keys(firstAvailableOption).forEach((key) => {
      if (key !== "quantity") {
        const value = firstAvailableOption[key as keyof typeof firstAvailableOption];
  ...
  return (
    <Link to={`/product/${product.id}`} className="block h-full" onClick={handleCardClick}>
      {cardContent}
    </Link>
  );
};
